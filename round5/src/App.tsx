...
import { login, logout, checkUserLoggedState, UserPayload, userSign, loggedin } from './store/userSlice';
import {
  VALIDATOR_EMAIL,
  VALIDATOR_MAXLENGTH,
  VALIDATOR_MINLENGTH,
...
function App() {
  ...
  useEffect(() => {
    ...
    if(typeof userInfo !== 'string' && userInfo !== null){
      dispatch(checkUserLoggedState(userInfo))
    }
  },[dispatch,userData.expirationDate,userData])
  ...
  return (
    <div className="w-full mx-auto my-0">
      ...
      <div>
        {userData.email && <div>you are logged in.{userData.email}</div>}
